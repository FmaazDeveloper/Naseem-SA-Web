                            ...
                            <tbody class="table-group-divider">
                                @foreach ($tickets as $ticket)
                                    <tr>
                                        ...
                                        <td>{{ $ticket->contact_reason->name }}</td>
                                        <td>{{ $ticket->title }}</td>
                                        <td><a href="{{ url($ticket->ticket_file ?? '#') }}" target="_blanck">View</a></td>
                                        <td>{{ $ticket->status }}</td>
                                        <td><a href="{{ url($ticket->answer_file ?? '#') }}" target="_blanck">View</a></td>
                                        <td>
                                            <small>
                                                <p>{{ 'Created : ' . $ticket->created_at }}</p>
                                                <p>{{ 'Updated : ' . $ticket->updated_at }}</p>
                                                <p>{{ 'Closed : ' . $ticket->closed_at }}</p>
                                            </small>
                                        </td>
                                        <td>
                                            ...
                                            <form style="display: inline" method="post"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    onclick="return confirm('Are you sure to delete Ticket ID #  {{ $ticket->id }}?')"
